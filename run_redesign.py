...
# Replace everything from <div id="siswaLoginPage" to <!-- ===== STUDENT RIWAYAT PAGE
pattern = re.compile(r'<div id=\"siswaLoginPage\".*?<!-- ===== STUDENT RIWAYAT PAGE', re.DOTALL)
...
# Modify Javascript using regex
js_pattern = re.compile(r"btnSiswaLogout\.addEventListener\('click', \(\) => \{.*?\}\);", re.DOTALL)
...
# Replace errors
err1 = re.compile(r"siswaLoginError\.textContent = 'NISN tidak ditemukan.*?siswaLoginError\.style\.display = 'block';", re.DOTALL)
new_err1 = '''const errText = document.getElementById('siswaLoginErrorText');
...
html = err1.sub(new_err1, html)

err2 = re.compile(r"siswaLoginError\.textContent = 'Terjadi kesalahan.*?siswaLoginError\.style\.display = 'block';", re.DOTALL)
new_err2 = '''const errText = document.getElementById('siswaLoginErrorText');
          if (errText) errText.textContent = 'Terjadi kesalahan jaringan. Coba lagi.';
          siswaLoginError.style.display = 'flex';
          document.getElementById('inputNISN').classList.add('border-red-500', 'ring-4', 'ring-red-500/20');'''
html = err2.sub(new_err2, html)

err3 = re.compile(r"siswaLoginError\.style\.display = 'none';")
new_err3 = '''siswaLoginError.style.display = 'none';
        document.getElementById('inputNISN').classList.remove('border-red-500', 'ring-4', 'ring-red-500/20');'''
html = err3.sub(new_err3, html)
