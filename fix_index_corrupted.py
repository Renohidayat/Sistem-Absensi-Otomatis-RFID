...
target_corrupted = """          loadSiswaRiwayat(dbService);
        } catch (err) {
          ...
          tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem;color:#dc2626;">Gagal memuat data.</td></tr>';
        }
      }"""

replacement = """          loadSiswaRiwayat(dbService);
        } catch (err) {
          const errText = document.getElementById('siswaLoginErrorText');
          if (errText) errText.textContent = 'Terjadi kesalahan jaringan. Coba lagi.';
          siswaLoginError.style.display = 'flex';
...
if target_corrupted in html:
    html = html.replace(target_corrupted, replacement)
    with codecs.open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print('Fixed successfully!')
else:
    print('Corrupted block not found! Dumping context:')
